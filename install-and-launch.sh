...
if [ "$interactive" == "true" ] && [ -z "$sublime_host" ]; then
    ...
    printf "Please specify the hostname or IP address of where you're deploying Sublime.\n\n"
...
if ! sublime_host=$sublime_host skip_preflight=true interactive=$interactive auto_updates=$auto_updates bash <(curl -s https://raw.githubusercontent.com/sublime-security/sublime-platform/josh.setup_script/launch-sublime-platform.sh); then
